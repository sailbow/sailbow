...
import { User, getUser } from 'util/user/User';
import { LocalStorageKeys, LS } from 'util/localstorage/LocalStorage';
import { login, authorize, Provider } from './Auth.Service';
...
interface AuthStoreActionApis {
    ...
    getLocalUser: () => void;
    login: (provider: Provider, state: string) => Promise<string | null>;
...
export const useAuthStore = (): [AuthStoreState, AuthStoreActionApis] => {
    ...
    const actionApis: AuthStoreActionApis = {
        ...
        login: async (provider, urlState) => {
            try {
                const redirectUrl = await login(provider, urlState);
