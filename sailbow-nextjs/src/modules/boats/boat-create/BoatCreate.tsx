...
import { BoatBanner, BannerType, CreateBoat, EditBoat, CrewMemberRole } from '@/modules/boats/Boat.Types';
...
export const BoatCreate: FunctionComponent = () => {
    ...
    const { mutate: boatCreate } = api.dock.createBoat.useMutation({
        onSuccess: (data: z.infer<typeof CreateBoatContract.responseSchema>) => {
            router.push(`/dock/${data.id}`);
        }
    });
